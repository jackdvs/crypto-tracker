...
class Coins extends Component<Props> {
    ...
    public static getFormattedPrice(coin: ICoin): string {
    ...
    public static getStrFormattedPrice(price: number): string {
        const _price = price;
        const __price = _price < 0.1 ? _price < 0.01 ? _price.toFixed(6) : _price.toFixed(4) : _price.toFixed(2)
        const currencyInfo = currencyFormatter.findCurrency("USD");
        return currencyInfo.symbolOnLeft ? `${currencyInfo.symbol}${__price }` : `${__price }${currencyInfo.symbol}`
    }
